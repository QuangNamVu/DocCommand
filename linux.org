...
** copy large file to another partition
  dd if=/mnt/windows/Ghost\ W7\ By\ RouJi\ Full\ Driver\(1\).GHO of=/tmp/Ghost7.GHO conv=notrunc #copy file cp
 ( head -10 input.txt ; echo '=====' ) > output.txt
** create live usb
$  sudo dd bs=4M if=/home/nam/os/archlinux-2018.07.01-x86_64.iso of=/dev/sdb status=progress oflag=sync
599785472 bytes (600 MB, 572 MiB) copied, 114,131 s, 5,3 MB/s
143+1 records in
143+1 records out
601882624 bytes (602 MB, 574 MiB) copied, 114,782 s, 5,2 MB/s
...
** get size all image
identify -format "%wx%h\n" ~/data/data_khanh_4k5/single_img_resize/motorbike/*.jpg echo "\n"
** resize all image in file
for file in *.jpg; do convert $file -resize 64x128! resize-$file  && rm $file; done
