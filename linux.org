...
** change user name in file
rsync -vuar root_file/ new_user_name_file/ 
rsync -vuar ~/darknet/gvc_backup_nam/yolov3-voc_final.weights ~/Documents/checkpoint/gvc_final.weights
* Shell
...
** RAM
   ...
   free -m
** Speed test
   curl -s https://raw.ghttp://192.168.1.9/GVCgit/AI_vehicle.gitithubusercontent.com/sivel/speedtest-cli/master/speedtest.py | python -
** version linux
   cat /proc/versio
   cat /etc/*-release
** disk space
